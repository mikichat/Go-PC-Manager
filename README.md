...
#### 2. 윈도우 서비스 등록 (백그라운드 실행)
PC 부팅 시 에이전트가 자동으로 실행되도록 윈도우 서비스로 등록합니다.

**통합 관리 도구 사용 (권장):**
```bash
setup.exe
```
1. `setup.exe` 파일 하나만 있으면 됩니다. (`agent.exe`가 내장되어 있습니다.)
2. 실행하면 자동으로 관리자 권한을 요청합니다.
3. 메뉴에서 **[1] 에이전트 서비스 설치 및 시작**을 선택하세요.
   - 기본 설치 경로: `C:\Program Files\GoPCManager`
   - 해당 경로에 파일이 자동으로 추출되고 서비스가 등록됩니다.
4. 제거하려면 **[2] 에이전트 서비스 중지 및 제거**를 선택하세요.
   - 서비스가 제거되고 설치 폴더도 삭제됩니다.

**개발자용 빌드 방법:**
```bash
build_all.bat
```
* `agent`, `setup`, `server`를 순서대로 빌드합니다.
* `setup.exe`를 빌드하려면 반드시 `agent.exe`가 먼저 빌드되어 있어야 합니다.
